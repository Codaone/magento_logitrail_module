...
class Codaone_Logitrail_Model_Observer  extends Varien_Event_Observer {
    ...
    public function saveProduct($observer) {
       ...
       $result = Mage::getModel('logitrail/logitrail')->addProducts(array($observer->getEvent()->getProduct()->getId()));
       if ($result === true) {
             
          Mage::getSingleton('core/session')->addSuccess($helper->__('Product successfully added/updated to Logitrail')); 
        } else {
            Mage::getSingleton('core/session')->addError($helper->__('Error: Adding product failed: ') . $helper->escapeHtml($result));
        }
